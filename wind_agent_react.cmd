...
set "TUNNEL_TITLE=wind-agent-react-tunnel-%LOCAL_PORT%"
set "WEB_TITLE=wind-agent-react-next-v1-%UI_PORT%"
set "WEB_BOOT=%ROOT_DIR%tmp_start_react_next_v1.cmd"
...
echo [INFO] Starting Next.js v1 UI on port %UI_PORT% ...
start "%WEB_TITLE%" cmd /k ""%WEB_BOOT%""

echo [INFO] Waiting for V1 page...
powershell -NoProfile -Command "$ok=$false; for($i=0; $i -lt 120; $i++){ try { $r=Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:%UI_PORT%/v1' -TimeoutSec 2; if($r.StatusCode -ge 200){ $ok=$true; break } } catch {}; Start-Sleep -Seconds 1 }; if($ok){ exit 0 } else { exit 1 }"
if errorlevel 1 (
  echo [ERROR] Next.js page /v1 not reachable.
  echo [ERROR] Check window: %WEB_TITLE%
  if not defined NO_PAUSE pause
  exit /b 1
)

echo [OK] Open: http://127.0.0.1:%UI_PORT%/v1
start "" "http://127.0.0.1:%UI_PORT%/v1"
powershell -NoProfile -Command "Start-Process 'http://127.0.0.1:%UI_PORT%/v1'" >nul 2>nul

echo [DONE] React V1 UI started.
