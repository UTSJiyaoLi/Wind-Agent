    ...
    *,
    ...
    with tracer.span(
        ...
        if mode == "wind_agent":
            ...
            planner_cfg = {
                "base_url": str(getattr(runtime.args, "planner_base_url", "") or getattr(runtime.args, "orchestrator_base_url", "")),
                "model": str(getattr(runtime.args, "planner_model", "") or getattr(runtime.args, "orchestrator_model", "")),
                "api_key": str(getattr(runtime.args, "planner_api_key", "") or getattr(runtime.args, "orchestrator_api_key", "EMPTY")),
                "timeout_seconds": int(
                    getattr(runtime.args, "planner_timeout_seconds", 60)
            ...
            workflow_results = list(agent_result.get("workflow_results") or [])
            analysis = agent_result.get("analysis") if isinstance(agent_result.get("analysis"), dict) else {}
            if isinstance(analysis.get("batch_results"), list):
                for _item in analysis.get("batch_results") or []:
                    if not isinstance(_item, dict):
                        continue
                    if str(_item.get("tool") or "") != "analyze_typhoon_map":
                        continue
                    _res = _item.get("result") if isinstance(_item.get("result"), dict) else {}
                    _ms = _res.get("map_spec") if isinstance(_res, dict) else None
                    if isinstance(_ms, dict) and _ms.get("center"):
